<x-layout :title="$pageTitle">
  ...
  <div class="mt-6 flex items-center justify-end gap-x-6">
    <a href="/blog/create" class="rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-xs hover:bg-indigo-500 focus-visible:outline-2 focus-visible:outline-offset-2
     focus-visible:outline-indigo-600">Create
    </a>
  </div>
  ...
  @foreach ($posts as $post)
    <div class="flex justify-between items-center border border-gray-200 px-4 py-6 my-2">
      ...
      <div>
        ...
        <form method="post" action="/blog/{{ $post->id }}" onsubmit="return confirm ('Are you sure, this can not be reveresed?')">
            @method('DELETE')
            @csrf
